...
<body>
    <div class="col-md-6 order-md-2 d-flex">
        <a class="Ruangan1" href="historyRuangan">
            <i class="fa-solid fa-door-open mr-2"></i> Meeting Room
        </a>
        <a class="Driver" href="$historyDriver">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table role="table" aria-busy="false" aria-colcount="6" class="table b-table table-striped table-hover table-borderless border b-table-fixed b-table-stacked-sm custom-table" id="__BVID__56">
                    ...
                    <tbody>
                    <?php if(isset($historyDriver) && !empty($historyDriver)): ?>
            <?php foreach($historyDriver as $index => $item): ?>
                <tr>
                    <td style="text-align: center;"><?= $index + 1; ?></td>
                    <td style="text-align: center;"><?= esc($item['tanggal']); ?></td>
                    <td style="text-align: center;"><?= esc($item['destinasi']); ?></td>
                    <td style="text-align: center;"><?= esc($item['nama_driver']); ?></td>
                    ...
                    <?php else: ?>
                        <!-- Anda bisa menambahkan default atau penanganan status lainnya di sini -->
                        <button style="padding: 5px 10px; border: none; cursor: default; border-radius: 10px;">Tidak Diketahui</button>
                    <?php endif; ?>
                    </td>
                    <td class="small-text" style="text-align: center;">
                        <?php if ($item['status'] == 'setuju'): ?>
                        <a href="<?= base_url('user/batalkanBookingDriver/'.$item['id_booking_driver']); ?>" onclick="return confirm('Apakah Anda yakin ingin membatalkan booking ini?');" class="btn btn-secondary btn-sm">Batal</a> 
